...
cleanup_environment() {
    ...
    if [ -f "docker-compose.yml" ] || [ -f "docker-compose.yaml" ]; then
        ...
        elif sg docker -c "docker compose down --remove-orphans --volumes" 2>/dev/null; then
...
EOF
    ...
    cp ../lib/kafka-connect-mqtt-assembly-10.0.0.jar ./kafka/plugins/ 2>/dev/null || true
...
build_and_run() {
    ...
    # Build Scala/SBT project if build.sbt exists
    if [ -f "build.sbt" ]; then
    ...
    if [ -f "requirements.txt" ]; then
        ...
        if [ -f "main.py" ]; then
            echo
            read -p "Do you want to run main.py? (y/n): " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            echo
            read -p "Do you want to run app.py? (y/n): " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                log_info "Running Python application..."
                python app.py
...
init-kafka-plugins() {
    log_info "Initializing Kafka Connect plugins..."
    
    PLUGIN_DIR="./kafka/plugins"
    
    if [ ! -d "$PLUGIN_DIR" ]; then
        mkdir -p "$PLUGIN_DIR"
    fi
    
    # Example: Downloading Debezium MySQL connector
    if [ ! -f "$PLUGIN_DIR/debezium-connector-mysql.jar" ]; then
        log_info "Downloading Debezium MySQL connector..."
        wget -qO "$PLUGIN_DIR/debezium-connector-mysql.jar" "https://repo1.maven.org/maven2/io/debezium/debezium-connector-mysql/1.9.7.Final/debezium-connector-mysql-1.9.7.Final-plugin.tar.gz"
        tar -xzf "$PLUGIN_DIR/debezium-connector-mysql.jar" -C "$PLUGIN_DIR"
        rm "$PLUGIN_DIR/debezium-connector-mysql.jar"
        log_success "Debezium MySQL connector downloaded and extracted"
    else
        log_success "Debezium MySQL connector already exists"
    fi
...
# If the script fail with permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock
# Please ensure that your user is added to the Docker group with the following command:
# sudo usermod -aG docker $USER
# Run newgrp docker to take effect
# Restart your terminal or log out and back in
# Install mqtt-kafka-connect with
# git clone https://github.com/johanvandevenne/kafka-connect-mqtt.git
# cd kafka-connect-mqtt
# mvn clean install
# Mosquitto may take time to spin up, please be patient
# Download the MQTT connector https://github.com/lensesio/stream-reactor/releases